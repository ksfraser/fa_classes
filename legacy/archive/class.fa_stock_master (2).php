...
| editable           | tinyint(1)   | NO   |     | 0       |       |
	...
	/**//*******************************************************
	*	Grab the info for 1 stock_id
	*
	* @since 20250227
	* 	
	* @param string stock_id
	* @param bool only active
	* @returns array stock_id details
	*************************************************************/
	function getStock_ID( $stock_id, $active_only = true )
	{
		$sql = "select * from " . TB_PREF . "stock_master";
		$sql .= " where stock_id = '$stock_id'";
		
		$result = db_query( $sql , "Couldn't select stock_id" );
		while( ( $row = mysql_fetch_array( $result ) ) != null )
		{
			//There should only be 1 - it's a prikey
			return $row;
		}
	}
