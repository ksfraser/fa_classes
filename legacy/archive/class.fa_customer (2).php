...
require_once( __DIR__ . '/class.fa_crm_persons.php' );
require_once( __DIR__ . '/class.fa_crm_contacts_model.php' );
require_once( __DIR__ . '/class.fa_debtors_master_model.php' );
...
class fa_customer extends table_interface
{
	...
	protected $payment_terms = "4";	//Cash					//fa_cust							//debtors_master
	...
	protected $area = "2";		//CANADA				//fa_cust			//cust_branch
	...
	function __construct( $caller = null )
	{
		...
		$this->debtors_master = new fa_debtors_master_model();	//fa_debtors_master
		$this->crm_contacts = new fa_crm_contacts_model();		//fa_crm_contacts
		$this->crm_persons = new fa_crm_persons_model();		//fa_crm_persons
	...
	*********************************************************************
	...
	{
		return $this->debtors_master->get_keys_search( $key );
	}
	/**//*****************************************************************
	...
	* @returns bool did we find a customer, sets our values
	*********************************************************************/
	function searchCustomersByName()
	{
		...
		$this->debtors_master->set( "name", $this->get( "CustName" ) );
		$b_res = $this->debtors_master->searchCustomersByName();
		//var_dump( __FILE__ . "::" . __LINE__ );
		//var_dump( $b_res );
		//var_dump( __FILE__ . "::" . __LINE__ );
		//var_dump( $this->debtors_master );
/** debtor_no is being set by searchCustomersByName
		if( $b_res )
		{
			if( $this->debtors_master->get( "debtors_count" ) == 1 )
			{
				$this->set( "debtor_no", $this->debtors_master->get( "debtor_no" ) );
			}
		}
*/
/*
...
*/
	...
	*
	*	TODO:
	*		Move into debtor_master where it belongs!
	 *
	 * @return bool
	 *
	 * *******************************************************************/
	...
	*******************************************************/
	function add_branch()
	{
		...
		//$branch = new fa_cust_branch();
        	...
        	$branch->set( "address", $this->address );
	...
	/**//*********************************************************
	* Add a person to the CRM_PERSON table
	*
	* TODO: rewrite to use ->crm_person
	*
	* @param none
	* @return int
	**************************************************************/
	...
	/**//*********************************************************
	* Add a person to the CRM_CONTACT table
	*
	* TODO: rewrite to use ->crm_XXXXXX
	*
	* @param string person type (e.g. cust_branch, customer)
	* @param string actions e.g. general, delivery, order, etc
	* @param int selected branch or customer ID
	* @param in person ID
	* @return int
	**************************************************************/
	private function add_crm_contact( $type, $action, $id )
	{
                return add_crm_contact( $type, $action, $id, $this->person_id);
	}
	/*****************************************************************//**
	 ...
	 *
	 *
	 * @return array the data returned from get_customer
	 *
	 * *******************************************************************/
	...
	*		customer_list queries debtor_master.
	*
	* @param string var label
	* @param int selected_id
	* @param bool submit the selection on change
	* @returns string HTML drop down list string
	*************************************************************************/
	function customer_list_dropdown( $varname, $selected_id = null, $submit_on_change = null )
	{
		//require_once( $path_to_root . "/includes/ui/ui_lists.inc" );
		$html =  customer_list( $varname, $selected_id, false, $submit_on_change);
		return $html;
	}
	/**//*******************************************************************
	...
	*
	*	native includes/ui/ui_lists.inc
	*	Should be calling cust_branch for the list... fcn queries cust_branch
	...
	* @param int selected_id
	* @param book submit on change
	* @returns string HTML drop down list string
	*************************************************************************/
	function customer_branches_dropdown( $debtor_no, $varname, $selected_id = null, $submit_on_change = null )
	{
		//require_once( $path_to_root . "/includes/ui/ui_lists.inc" );
		$html = customer_branches_list($debtor_no, $varname . "_" . $debtor_no, $selected_id, false, true, $submit_on_change);
		return $html;
	}
	/****************************************************************************
	 ******************** Import PAYPAL *****************************************
