...
{
	protected $customer_id;		//!<int debtor_no
	protected $alloc;		//!<class allocation
	protected $SInvoice;		//!<int Sales Invoice (type_no)
	protected $dflt_act;
	...
	*
	* Taken from bank_import working code
	* 	
	* @since 20250314
	*
	* @param none use internal
	* @returns none sets internal
	*********************************************************************
	function import_write_customer_payment()
	{
		$field_list = array( 'trans_no', 'customer_id', 'Branch_ID', 'bank_account',
				'trans_date', $reference', 'amount', 'discount', 'memo_', 'rate', 'charge', 'bank_amount', 'trans_type');
		foreach( $field_list as $field )
		{
			if( $isset( $this->$field ) )
			{
				throw new Exception( "required field ::" . $field . ":: is not set", KSF_VAR_NOT_SET );
			}
		}
		$this->payment_id = my_write_customer_payment( $this->trans_no, $this->customer_id, $this->Branch_ID, $this->bank_account,
					$this->trans_date, $reference, $this->amount, $this->discount, $this->memo_, $this->rate, user_numeric($this->charge), $this->bank_amount, $this->trans_type);
		if( isset( $this->SInvoice ) )
		{
				add_cust_allocation( $this->amount, ST_CUSTPAYMENT, $this->payment_id, ST_SALESINVOICE, $this->SInvoice, $this->customer_id, $this->trans_date);
                        	update_debtor_trans_allocation(ST_SALESINVOICE, $this->SInvoice, $this->customer_id);
                        	update_debtor_trans_allocation(ST_CUSTPAYMENT, $this->payment_id, $this->customer_id);
		}
	}
	/**//***************************************************
